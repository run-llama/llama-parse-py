...
from llama_cloud._files import to_httpx_files, deepcopy_with_paths, async_to_httpx_files
from llama_cloud._utils import extract_files
...
def test_string_works() -> None:
    result = to_httpx_files({"file": str(readme_path)})
    print(result)
    assert result == IsDict({"file": IsTuple("README.md", IsBytes())})
